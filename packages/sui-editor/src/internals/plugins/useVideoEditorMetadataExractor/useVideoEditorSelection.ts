...
import { VideoEditorPlugin } from '../../models';
import { FileId } from '../../../models';

import {
  UseVideoEditorSelectionInstance,
  UseVideoEditorSelectionSignature,
} from './useVideoEditorSelection.types';
import { convertSelectedItemsToArray, getLookupFromArray } from './useVideoEditorSelection.utils';

export const useVideoEditorSelection: VideoEditorPlugin<UseVideoEditorSelectionSignature> = ({
  instance,
  params,
  models,
}) => {
  ...
  const selectItem: UseVideoEditorSelectionInstance['selectItem'] = (
    event,
    itemId,
  ...
  const expandSelectionRange = (event: React.SyntheticEvent, itemId: string) => {
    if (lastSelectedItem.current != null) {
      // const [start, end] = findOrderInTremauxVideoEditor(instance, itemId, lastSelectedItem.current);
...
useVideoEditorSelection.models = {
  selectedItems: {
...
useVideoEditorSelection.getDefaultizedParams = (params) => ({
  ...params,
...
useVideoEditorSelection.params = {
