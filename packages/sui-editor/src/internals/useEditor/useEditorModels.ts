...
export const useEditorModels = <TSignatures extends readonly EditorAnyPluginSignature[]>(
  ...
  const [modelsState, setModelsState] = React.useState<{ [modelName: string]: any }>(() => {
    ...
    plugins.forEach((plugin: EditorPlugin<EditorAnyPluginSignature>) => {
      if (plugin.models) {
    ...
    Object.entries(modelsRef.current).forEach(([modelName, model]) => {
        ...
        if (model.isControlled !== (controlled !== undefined)) {
          console.error(
            [
              ...
              }controlled ${modelName} state of Editor to be ${
                model.isControlled ? 'un' : ''
              }controlled.`,
      ...
      React.useEffect(() => {
        if (!model.isControlled && defaultValue !== newDefaultValue) {
          console.error(
            [
              `SUI X: A component is changing the default ${modelName} state of an uncontrolled Editor after being initialized. ` +
                `To suppress this warning opt to use a controlled Editor.`,
            ].join('\n'),
          );
        }
