...
const TimelinePlayer = ({ timelineState, editorData, autoScrollWhenPlay, scale = 1, scaleWidth = 160, startLeft = 20, }) => {
    ...
    const handleRateChange = (event) => {
        ...
        timelineState.current.setPlayRate(event.target.value);
    };
    // Time display
    const timeRender = (renderTime) => {
        ...
        return React.createElement(React.Fragment, null, `${min}:${second}.${float.replace('0.', '')}`);
    };
    const handleStart = () => {
    ...
    return (React.createElement(PlayerRoot, { className: "timeline-player" },
        React.createElement(IconButtonControlRoot, { size: 'small', className: "play-control", onClick: handleStart },
            React.createElement(SkipPreviousIcon, null)),
        React.createElement(IconButtonControlRoot, { size: 'small', className: "play-control", onClick: handlePlayOrPause }, isPlaying ? React.createElement(PauseIcon, null) : React.createElement(PlayArrowIcon, null)),
        React.createElement(IconButtonControlRoot, { size: 'small', className: "play-control", onClick: handleEnd },
            React.createElement(SkipNextIcon, null)),
        React.createElement(TimeRoot, { className: "time" }, timeRender(time)),
        React.createElement(RateControlRoot, { className: "rate-control" },
            React.createElement(Select, { size: "small", labelId: "speed-label", defaultValue: 1, label: "Speed", variant: "standard", onChange: handleRateChange, sx: { height: '28px', fontSize: '12px' }, renderValue: (selected) => {
                    return selected;
                } }, Rates.map((rate) => (React.createElement(MenuItem, { sx: { height: '28px', fontSize: '12px' }, key: rate, value: rate }, `${rate.toFixed(1)}x`)))))));
};
/*
import * as React from 'react';
