...
const Timeline = React.forwardRef((props, ref) => {
    ...
    const { style } = props;
    const { effects, editorData: data, scrollTop, autoScroll, hideCursor, disableDrag, scale, scaleWidth, startLeft, minScaleCount, maxScaleCount, onChange, engine, autoReRender = true, onScroll: onScrollVertical, } = checkedProps;
    const engineRef = React.useRef(engine || new TimelineEngine());
    const domRef = React.useRef();
    const areaRef = React.useRef();
    const scrollSync = React.useRef();
    // Editor data
    ...
    const handleSetScaleCount = (value) => {
    ...
    /** setUp scrollLeft */
    const handleDeltaScrollLeft = (delta) => {
        // 当超过最大距离时，禁止自动滚动
        const data = scrollSync.current.state.scrollLeft + delta;
        if (data > scaleCount * (scaleWidth - 1) + startLeft - width)
            return;
        scrollSync.current &&
    ...
    return (React.createElement(TimelineRoot, { ref: domRef, style: style, className: `${PREFIX} ${disableDrag ? PREFIX + '-disable' : ''}` },
        React.createElement(ScrollSync, { ref: scrollSync }, ({ scrollLeft, scrollTop, onScroll }) => (React.createElement(React.Fragment, null,
            React.createElement(TimeArea, { ...checkedProps, timelineWidth: width, disableDrag: disableDrag || isPlaying, setCursor: handleSetCursor, cursorTime: cursorTime, editorData: editorData, scaleCount: scaleCount, setScaleCount: handleSetScaleCount, onScroll: onScroll, scrollLeft: scrollLeft }),
            React.createElement(EditArea, { ...checkedProps, timelineWidth: width, ref: (ref) => (areaRef.current = ref?.domRef.current), disableDrag: disableDrag || isPlaying, editorData: editorData, cursorTime: cursorTime, scaleCount: scaleCount, setScaleCount: handleSetScaleCount, scrollTop: scrollTop, scrollLeft: scrollLeft, setEditorData: handleEditorDataChange, deltaScrollLeft: autoScroll && handleDeltaScrollLeft, onScroll: (params) => {
                    onScroll(params);
                    onScrollVertical && onScrollVertical(params);
                } }),
            !hideCursor && (React.createElement(Cursor, { ...checkedProps, timelineWidth: width, disableDrag: isPlaying, scrollLeft: scrollLeft, scaleCount: scaleCount, setScaleCount: handleSetScaleCount, setCursor: handleSetCursor, cursorTime: cursorTime, editorData: editorData, areaRef: areaRef, scrollSync: scrollSync, deltaScrollLeft: autoScroll && handleDeltaScrollLeft })))))));
});
Timeline.propTypes = {
    // ----------------------------- Warning --------------------------------
     ...
     */
    editorData: PropTypes.arrayOf(PropTypes.shape({
