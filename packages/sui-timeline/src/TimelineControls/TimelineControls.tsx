...
function Volume({ disabled }) {
  ...
  const getIcon = (isMute: boolean, volume: number) => {
    ...
    if (isMute) {
      ...
      icon = <VolumeMute onClick={toggleMute} sx={{ left: '-4px', position: 'relative', mr: '4px!important', ...cursor }} />;
    } else if (volume < 70) {
      icon = <VolumeDown sx={{ left: '-2px', position: 'relative', mr: '4px!important', ...cursor }} onClick={toggleMute} />;
    }
    return icon;
  }
    ...
    <PlayerRoot id={'timeline-controls'} className="timeline-player" ref={ref}>
      ...
      <div style={{display: 'flex', flexDirection: 'row'}}>
        ...
        <TimeRoot disabled={!!disabled} className={`MuiFormControl-root MuiTextField-root ${disabled ? 'Mui-disabled' : ''}`}>
          <div className={`MuiInputBase-root MuiOutlinedInput-root MuiInputBase-colorPrimary ${disabled ? 'Mui-disabled' : ''} MuiInputBase-formControl MuiInputBase-sizeSmall css-qp45lg-MuiInputBase-root-MuiOutlinedInput-root`}>
            <Box aria-invalid="false" id="time" sx={(theme) => ({ color: `${disabled ? theme.palette.text.disabled : theme.palette.text.primary}!important` })} className={`MuiInputBase-input MuiOutlinedInput-input ${disabled ? 'Mui-disabled' : ''} MuiInputBase-inputSizeSmall css-r07wst-MuiInputBase-input-MuiOutlinedInput-input`}>
              {timeRender(time)}
            </Box>
          </div>
        </TimeRoot>
