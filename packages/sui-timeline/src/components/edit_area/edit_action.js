...
export const EditAction = ({ editorData, row, action, effects, rowHeight, scale, scaleWidth, scaleSplitCount = 10, startLeft = 20, gridSnap, disableDrag, scaleCount, maxScaleCount, setScaleCount, onActionMoveStart, onActionMoving, onActionMoveEnd, onActionResizeStart, onActionResizeEnd, onActionResizing, dragLineData, setEditorData, onClickAction, onClickActionOnly, onDoubleClickAction, onContextMenuAction, getActionRender, handleTime, areaRef, deltaScrollLeft, }) => {
    const rowRnd = useRef();
    ...
    const rightLimit = Math.min(maxScaleCount * scaleWidth + startLeft, // 根据maxScaleCount限制移动范围
    ...
    const handleScaleCount = (left, width) => {
    ...
    const handleDragEnd = ({ left, width }) => {
        // 计算时间
        const { start, end } = parserTransformToTime({ left, width }, { scaleWidth, scale, startLeft });
        // setData
    ...
    const handleResizing = (dir, { left, width }) => {
        isDragWhenClick.current = true;
        if (onActionResizing) {
            const { start, end } = parserTransformToTime({ left, width }, { scaleWidth, scale, startLeft });
    ...
    const handleResizeEnd = (dir, { left, width }) => {
        // calculatingTime
        const { start, end } = parserTransformToTime({ left, width }, { scaleWidth, scale, startLeft });
        // 设置数据
        const rowItem = editorData.find((item) => item.id === row.id);
    ...
    return (React.createElement(RowDnd, { ref: rowRnd, parentRef: areaRef, start: startLeft, left: transform.left, width: transform.width, grid: (gridSnap && gridSize) || DEFAULT_MOVE_GRID, adsorptionDistance: gridSnap ? Math.max((gridSize || DEFAULT_MOVE_GRID) / 2, DEFAULT_ADSORPTION_DISTANCE) : DEFAULT_ADSORPTION_DISTANCE, adsorptionPositions: dragLineData.assistPositions, bounds: {
            left: leftLimit,
            right: rightLimit,
        }, edges: {
            left: !disableDrag && flexible && `.${prefix('action-left-stretch')}`,
            right: !disableDrag && flexible && `.${prefix('action-right-stretch')}`,
        }, enableDragging: !disableDrag && movable, enableResizing: !disableDrag && flexible, onDragStart: handleDragStart, onDrag: handleDrag, onDragEnd: handleDragEnd, onResizeStart: handleResizeStart, onResize: handleResizing, onResizeEnd: handleResizeEnd, deltaScrollLeft: deltaScrollLeft },
        React.createElement(Action, { onMouseDown: () => {
            ...
            }, className: prefix((classNames || []).join(' ')), style: { height: rowHeight } },
            getActionRender && getActionRender(nowAction, nowRow),
            flexible && React.createElement(LeftStretch, { className: prefix('action-left-stretch') }),
            flexible && React.createElement(RightStretch, { className: prefix('action-right-stretch') }))));
};
