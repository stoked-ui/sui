...
export const RowDnd = React.forwardRef(({ children, edges, left, width, start = DEFAULT_START_LEFT, grid = DEFAULT_MOVE_GRID, bounds = {
    left: Number.MIN_SAFE_INTEGER,
    right: Number.MAX_SAFE_INTEGER,
}, enableResizing = true, enableDragging = true, adsorptionDistance = DEFAULT_ADSORPTION_DISTANCE, adsorptionPositions = [], onResizeStart, onResize, onResizeEnd, onDragStart, onDragEnd, onDrag, parentRef, deltaScrollLeft, }, ref) => {
    ...
    const handleUpdateLeft = (left, reset = true) => {
    ...
    const handleUpdateWidth = (width, reset = true) => {
        if (!interactable.current || !interactable.current.target)
            return;
        reset && (deltaX.current = 0);
        const target = interactable.current.target;
    ...
    const handleResizeStart = (e) => {
        ...
        isAdsorption.current = false;
        initAutoScroll();
        let dir = e.edges?.right ? 'right' : 'left';
    ...
    //#endregion
    return (React.createElement(InteractComp, { interactRef: interactable, draggable: enableDragging, resizable: enableResizing, draggableOptions: {
            lockAxis: 'x',
            onmove: handleMove,
...
//# sourceMappingURL=row_rnd.js.map