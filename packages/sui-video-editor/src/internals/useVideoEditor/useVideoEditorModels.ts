...
import { VideoEditorCorePluginSignatures } from '../corePlugins';

/**
 * Implements the same behavior as `useControlled` but for several models.
 * The controlled models are never stored in the state, and the state is only updated if the model is not controlled.
 */
export const useVideoEditorModels = <TSignatures extends readonly VideoEditorAnyPluginSignature[]>(
  plugins: ConvertSignaturesIntoPlugins<readonly [...VideoEditorCorePluginSignatures, ...TSignatures]>,
  ...
  const [modelsState, setModelsState] = React.useState<{ [modelName: string]: any }>(() => {
    ...
    plugins.forEach((plugin: VideoEditorPlugin<VideoEditorAnyPluginSignature>) => {
      if (plugin.models) {
    ...
    Object.entries(modelsRef.current).forEach(([modelName, model]) => {
      ...
      React.useEffect(() => {
        if (model.isControlled !== (controlled !== undefined)) {
          console.error(
            [
              `SUI X: A component is changing the ${
                model.isControlled ? '' : 'un'
              }controlled ${modelName} state of VideoEditor to be ${
                model.isControlled ? 'un' : ''
              }controlled.`,
      ...
      React.useEffect(() => {
        if (!model.isControlled && defaultValue !== newDefaultValue) {
          console.error(
            [
              `SUI X: A component is changing the default ${modelName} state of an uncontrolled VideoEditor after being initialized. ` +
                `To suppress this warning opt to use a controlled VideoEditor.`,
            ].join('\n'),
          );
        }
