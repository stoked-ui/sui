...
import { UseVideoEditorBaseProps } from './useVideoEditor.types';
...
interface ExtractPluginParamsFromPropsReturnValue<
  TSignatures extends readonly VideoEditorAnyPluginSignature[],
  TProps extends Partial<UseVideoEditorBaseProps<TSignatures>>,
> extends UseVideoEditorBaseProps<TSignatures> {
...
export const extractPluginParamsFromProps = <
  TSignatures extends readonly VideoEditorPluginSignature<any>[],
  TProps extends Partial<UseVideoEditorBaseProps<TSignatures>>,
>({
...
>): ExtractPluginParamsFromPropsReturnValue<TSignatures, TProps> => {
  ...
  const defaultizedPluginParams = plugins.reduce(
    (acc, plugin: VideoEditorPlugin<VideoEditorAnyPluginSignature>) => {
