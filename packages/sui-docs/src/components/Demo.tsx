...
import { useCodeVariant } from './codeVariant';
import { useCodeStyling } from './codeStylingSolution';
// @ts-ignore
import constants from './constants';
import { useUserLanguage, useTranslate } from '../i18n';
import stylingSolutionMapping from './stylingSolutionMapping';
import { BrandingProvider, blue, blueDark, grey } from '../branding';
import DemoToolbarRoot from './DemoToolbarRoot';


const { CODE_VARIANTS, CODE_STYLING } = constants;
/**
...
function getDemoName(location: any) {
...
function useDemoData(codeVariant: any, demo: any, githubLocation: any, codeStyling: any) {
  const userLanguage = useUserLanguage();
  const router = useRouter();
  const { canonicalAs } = pathnameToLanguage(['en'], router.asPath);
...
function useDemoElement({ demoData, editorCode, setDebouncedError, liveDemoActive }: any) {
...
const Root = styled('div')<{noMargins: any}>(({ theme, noMargins }) => ({
  marginBottom: 0,
...
})<{hideToolbar: any, bg: any}>(({ theme, hideToolbar, bg }) => ({
  position: 'relative',
  margin: 'auto',
  display: 'flex',
...
const DemoRootJoy = joyStyled('div', {
  shouldForwardProp: (prop) => prop !== 'hideToolbar' && prop !== 'bg',
})<{hideToolbar: any, bg: any, theme?: Theme & { applyDarkStyles: any }}>(({ theme, hideToolbar, bg }) => ({
  position: 'relative',
  margin: 'auto',
  display: 'flex',
  justifyContent: 'center',
...
const DemoCodeViewer: any = styled(HighlightedCode)(() => ({
  '& pre': {
    margin: 0,
...
export default function Demo(props: any) {
  ...
  const Wrapper: any = demoData.productId === 'joy-ui' ? BrandingProvider : React.Fragment;
          ...
          <Collapse in={openDemoSource} unmountOnExit timeout={150}>
            ...
            {demoOptions.disableLiveEdit ? (
              ...
              <DemoEditor
                ...
                key={editorCode.isPreview.toString()}
