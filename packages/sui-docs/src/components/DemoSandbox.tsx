...
function getTheme(outerTheme: any) {
  const brandingDesignTokens = getDesignTokens(outerTheme.palette.mode) as ThemeOptions;
  const isCustomized =
    outerTheme.palette.primary?.main &&
    outerTheme.palette.primary.main !== (brandingDesignTokens.palette!.primary as SimplePaletteColorOptions).main;
...
const jss = create({
  plugins: [...jssPreset().plugins, rtl()],
  insertionPoint:
    typeof window !== 'undefined' ? insert : undefined,
});
