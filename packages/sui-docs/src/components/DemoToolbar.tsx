...
import { useSetCodeStyling, useCodeStyling } from './codeStylingSolution';
import { useTranslate } from '../i18n';
import stylingSolutionMapping from './stylingSolutionMapping';
...
const Button: any = styled(MDButton)(({ theme }) => ({
  height: 24,
...
const MenuItem = styled(MDMenuItem)<{ href?: string, target?: any, component?: any }>(({ theme }) => ({
 ...
 * @param {(index: number) => boolean} [options.isFocusableControl] In case certain controls become
 *   unfocusable
 * @param {number} [options.defaultActiveIndex]
 */
function useToolbar(controlRefs: any, options: any = {}) {
  const { defaultActiveIndex = 0, isFocusableControl = alwaysTrue } = options;
  const [activeControlIndex, setActiveControlIndex] = React.useState<number>(defaultActiveIndex);
  ...
  function findControlIndex(element: React.ElementType) {
    let controlIndex = -1;
    controlRefs.forEach((controlRef: any, index: number) => {
  ...
  function handleControlFocus(event: any) {
  ...
  function handleToolbarKeyDown(event: any) {
...
export interface DemoToolbarProps {
  ...
  onCodeOpenChange: any,
  onResetDemoClick: any,
...
export default function DemoToolbar(props: DemoToolbarProps) {
  ...
  const setCodeStyling = useSetCodeStyling() as ((...args: any[]) => {});
  ...
  const handleCodeLanguageClick = (event: any, clickedCodeVariant: any) => {
  ...
  const createHandleCodeSourceLink = (anchor: any, codeVariantParam: any, stylingSolution: any) => async () => {
    try {
      await copy(
  ...
  const isFocusableControl = React.useCallback(
    (index: any) => (codeOpen ? true : index !== 1 && index !== 2),
    [codeOpen],
  );
  ...
  const handleStylingSolutionChange = (eventStylingSolution: any) => {
  ...
  const handleStylingButtonClick = (event: any) => {
