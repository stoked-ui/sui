...
async function fetchNotifications(props?: any) {
  const { items } = props;
  if (process.env.NODE_ENV === 'development') {
    return items;
  }
  const response = await fetch(
    'https://raw.githubusercontent.com/stoked-ui/mono/main/docs/notifications.json',
  );
...
export default function Notifications() {
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      if (active) {
        ...
        const filteredNotifications: Array<any> = [
          /* {
            id: 0,
  ...
  const msgReducer = (arr) => arr ? arr.reduce((count, message) => ((lastSeen ? (message.id > lastSeen) : false) ? count + 1 : count), 0) : 0
  return (
    <React.Fragment>
      <Tooltip
        open={tooltipOpen}
      ...
      >
        ...
        >
          <Badge
            color="error"
            badgeContent={msgReducer(messageList)}
          >
