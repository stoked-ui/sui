...
const CodeVariantContext = React.createContext({
  ...
  setCodeVariant: (lang: string) => {
    if (process.env.DEPLOY_ENV === 'staging' || process.env.DEPLOY_ENV === 'pull-request') {
      console.warn(`set lang: ${lang}`)
    }
  },
  noSsrCodeVariant: undefined
});
...
export function CodeVariantProvider(props) {
  ...
  const contextValue = React.useMemo<{ codeVariant: any, noSsrCodeVariant: any, setCodeVariant: any}>(() => {
