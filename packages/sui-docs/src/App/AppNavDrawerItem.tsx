...
<{hasIcon: boolean, depth: number, subheader: any, expandable: boolean}>(({ theme, hasIcon, depth, subheader, expandable }) => {
  const color = {
...
})<{ open: boolean }>(({ open, theme }: { open: boolean, theme: Theme }) => ({
  fontSize: '1rem',
...
const StyledLi = styled('li', { shouldForwardProp: (prop) => prop !== 'depth' })(
  ({ theme, depth }: { theme: Theme, depth: number}) => ({
    display: 'block',
  ...
  return (
    <StyledLi {...other} depth={depth}>
      ...
      >
        ...
        {itemAlpha && <Chip label="Alpha" sx={sxChip(getChipColor(theme,'error'))} />}
