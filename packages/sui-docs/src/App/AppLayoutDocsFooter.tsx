...
// import SvgSuiLogotype from '../icon/SvgSuiLogotype';
...
/**
 * @typedef {import('@stoked-ui/docs').SuiPage} SuiPage
 * @typedef {import('docs/src/pages').OrderedSuiPage} OrderedSuiPage
 */

/**
 * @param {SuiPage[]} pages
 * @param {SuiPage[]} [current]
 * @returns {OrderedSuiPage[]}
 */
function orderedPages(pages, current = new Array()) {
  return pages
    .reduce((items, item) => {
...
async function submitFeedback(page, rating, comment, language, commentedSection, productId) {
  ...
  const resultSlack = await postFeedbackOnSlack({ ...data, commentedSection, productId });
  if (rating !== undefined) {
...
/**
 * @returns { { prevPage: OrderedSuiPage | null; nextPage: OrderedSuiPage | null } }
 */
...
const SPEACIAL_FEEDBACK_HASH: Array<any> = [{ hash: 'new-docs-api-feedback', text: 'New API content design' }];

export default function AppLayoutDocsFooter(props) {
  const { tableOfContents = new Array(), location, Logotype } = props;
  ...
  const hidePagePagination = activePage === null || !("ordered" in activePage);
...
AppLayoutDocsFooter.propTypes = {
  ...
  Logotype: PropTypes.elementType,
