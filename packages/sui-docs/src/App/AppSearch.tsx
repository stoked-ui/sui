...
function NewStartScreen() {
  const startScreenOptions: Array<any> = new Array<any>(
    {
      category: {
        name: 'Material UI',
      },
...
export default function AppSearch(props) {
  ...
  const facetFilterLanguage =
    pageContext.languages.indexOf(userLanguage) !== -1 ? `language:${userLanguage}` : `language:en`;
    ...
    if (isOpen) {
      ...
      if (searchInput) {
        const handleInput = (event) => {
          ...
          if (newStartScreen) {
            (newStartScreen as HTMLElement).style.display = event.target.value !== '' ? 'none' : 'grid';
          }
        };
  ...
  const optionalFilters: Array<any> = new Array();
