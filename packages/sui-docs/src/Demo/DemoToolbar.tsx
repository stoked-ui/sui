...
import { useSetCodeStyling, useCodeStyling } from '../utils/codeStylingSolution';
...
const MenuItem = styled(MDMenuItem)<{ component?: string, href?: string, target?: string }>(({ theme }) => ({
 ...
 */
function useToolbar(controlRefs, { defaultActiveIndex = 0, isFocusableControl = alwaysTrue }: {defaultActiveIndex: number, isFocusableControl: any }) {
  ...
  function handleToolbarKeyDown(event) {
    ...
    const focusableControls: Array<any> = new Array();
    controlRefs.forEach((controlRef, index) => {
...
export default function DemoToolbar(props) {
  ...
  const devMenuItems: Array<any> = new Array();
