...
function DemoError(props) {
  const { error, name, t, onResetDemoClick } = props;
...
export default function DemoErrorBoundary(props) {
  const [error, setError] = React.useState<Error>();

  function getDerivedStateFromError(demoError) {
    return setError(demoError);
  }
  if (error) {
    return <DemoError {...props } error={error} />;
  }
  try {
