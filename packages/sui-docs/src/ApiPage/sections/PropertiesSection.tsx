  ...
  return {
    ...
    children: [
      ...Object.entries(componentProps)
        .filter(([, propData]: [any, any]) => propData.description !== '@ignore')
        .map(([propName]) => ({
          text: propName,
...
export default function PropertiesSection(props: any) {
  const {
    properties,
  ...
  const formatedProperties = Object.entries(properties)
    ...
    .map(([propName, propData]: [any, any]) => {
      ...
      const signatureArgs = propData.signature?.describedArgs?.map((argName: any) => ({
        argName,
