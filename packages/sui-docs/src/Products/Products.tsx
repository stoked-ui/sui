...
export function getProductInfoFromPath(productPath: string): ProductInfo | null {
  try {

    // Resolve the full path to the package.json file
    const packageJsonPath = path.resolve(productPath, 'package.json');

    // Read the package.json file contents
    const packageJsonData = fs.readFileSync(packageJsonPath, 'utf-8');

    // Parse the JSON data
    const pkg = JSON.parse(packageJsonData);

    // Return the product info if available
    return pkg.product as ProductInfo;
  } catch (error) {
    console.error(`Error reading package.json from ${productPath}:`, error);
    throw new error;
  }
}

export function getProductInfo(productMeta: Record<string, string>) {
  const ProductIds: Array<string> = ['null', ...Object.keys(productMeta)];

  try {
    const ProductInfo = Object.keys(productMeta).reduce<Record<string, ProductInfo>>((acc, key) => {
      const value = productMeta[key];
      const productInfo = getProductInfoFromPath(value); // Call your function to generate ProductInfo
      if (productInfo) {
        acc[key] = productInfo;
      }
      return acc;
    }, {});
    return {ProductIds, ProductInfo };
  } catch (ex) {
    throw new Error(`getProductInfo failed: ${ex}`);
  }
}
