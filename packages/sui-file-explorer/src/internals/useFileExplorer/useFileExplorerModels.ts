...
export const useFileExplorerModels = <TSignatures extends readonly FileExplorerAnyPluginSignature[]>(
  ...
  const [modelsState, setModelsState] = React.useState<{ [modelName: string]: any }>(() => {
    ...
    plugins.forEach((plugin: FileExplorerPlugin<FileExplorerAnyPluginSignature>) => {
      if (plugin.models) {
    ...
    Object.entries(modelsRef.current).forEach(([modelName, model]) => {
        ...
        if (model.isControlled !== (controlled !== undefined)) {
          console.error(
            [
              ...
              }controlled ${modelName} state of FileExplorer to be ${
                model.isControlled ? 'un' : ''
              }controlled.`,
      ...
      React.useEffect(() => {
        if (!model.isControlled && defaultValue !== newDefaultValue) {
          console.error(
            [
              `SUI X: A component is changing the default ${modelName} state of an uncontrolled FileExplorer after being initialized. ` +
                `To suppress this warning opt to use a controlled FileExplorer.`,
            ].join('\n'),
          );
        }
