...
export interface UseFileExplorerIdInstance {
  /**
   * Get the id attribute (i.e.: the `id` attribute passed to the DOM element) of a fileExplorer item.
   * If the user explicitly defined an id attribute, it will be returned.
   * Otherwise, the method created a unique id for the item based on the FileExplorer View id attribute and the item `itemId`
