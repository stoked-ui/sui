...
exports.default = useIncId;
const React = __importStar(require("react"));
function useIncId(props) {
    ...
    const createId = React.useCallback((val) => {
        return `${incrementId}-${val.padStart(length, '0')}`;
    }, [incrementId, length]);
    const counterRef = React.useRef(0);
    const incrementFunc = (React.useCallback(() => {
        const nextId = String(counterRef.current);
        counterRef.current += 1;
        return createId(nextId);
    }, [createId]));
    incrementFunc.by = React.useCallback((step) => {
        const nextId = String(counterRef.current);
...
//# sourceMappingURL=useIncId.js.map