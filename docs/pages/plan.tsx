...
export default function Plan(props: InferGetStaticPropsType<typeof getStaticProps>) {
  const router = typeof window === "undefined" ? null : useRouter();
  const query = router === null ? props.query : router.query;
    ...
    router.push(
      {
        query: {
          ...router.query,
          tags: (Array.isArray(router.query.tags) ? router.query.tags : [router.query.tags])
            .filter((value) => value !== tag)  as any,
        },
      },
      undefined,
      { shallow: true }
          ...
          <SectionHeadline
            ...
            title={
              <Typography variant="h2" component="h1">
                Stay <GradientText>in the loop</GradientText> what I'm up to
              </Typography>
            }
          />
        </Section>
