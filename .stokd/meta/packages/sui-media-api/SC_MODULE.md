...
> **Generated:** 2026-05-05 (fresh) | **Last upgraded:** 2026-06-22 (meta v0.4.0 → v0.6.0) | **Prior upgrade:** 2026-05-21 (v0.3.0 → v0.4.0) | **Last refreshed:** 2026-06-06
> **Meta version:** 0.6.0
> **Verification (2026-06-22, v0.6.0 upgrade):** Re-verified against source. Controllers (`auth`/`media`/`uploads`/`health` — `rg "@Controller\("` returns exactly these four plus a doc-comment in `performance/caching.middleware.ts`), the `/media`, `/uploads`, and `/auth` routes, the `generate-thumbnail`/`generate-sprites` "Work Item 3.1" placeholders, upload constants (`DEFAULT_CHUNK_SIZE = 10 MiB`, `MAX_URLS_PER_REQUEST = 50`, `SESSION_EXPIRATION_DAYS = 7`, `MULTIPART_URL_EXPIRY = 1h`), the `^(video|image)\/[a-z0-9.+-]+$` MIME regex (`src/uploads/dto/index.ts:34`), the CORS default list, Swagger servers (`localhost:3001/v1`, `api.sui.stokd.cloud/v1`, `JWT-auth`), the JWT secret chain (`globalThis.Resource.JWT_SECRET` → `import('sst')` → `JWT_SECRET` env → `'dev-secret-change-me'`, `src/auth/auth.module.ts:19-27`), and the Lambda binary content-type list all confirmed unchanged. `MediaService` remains in-memory (`mediaStore: Map<string, MediaEntity>`, `src/media/media.service.ts:17`). The `src/blog`, `src/clients`, `src/users`, and `src/blog/dto` leftover directories remain on disk as *empty, untracked* folders. The e2e config `testRegex` (`.e2e-spec.ts$`) still does not match `test/uploads-e2e.spec.ts`, so that suite runs under the unit config (see SC_TEST.md §2).
