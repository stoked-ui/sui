...
> **Generated:** 2026-05-05 (refreshed 2026-05-21 for upgrade; timed refresh 2026-06-06 — corrected TanStack Query cache keys to match source, refreshed `MediaCard.tsx` line count; second 2026-06-06 pass — added the new `WebUserDirectChat` test suite to the inventory, fixed the `useActiveUploads` cache-key typo (`['activeUploads']` → `['uploads', 'active']`), re-verified LOC and the no-`exports`-map state; 0.6.0 upgrade 2026-06-22 — re-verified the 11 Jest suites, export surface, LOC for the six largest files, and `version` 0.1.0-alpha.5 against source; timed refresh 2026-07-02 — full verification pass with **no content changes required**: no source commit has touched `packages/sui-media` since 2026-06-22 (only `.axioms.md` gained AX-MOD-SUIMEDIA-008/009 in `13c8553b88`); re-ran the Jest suite (11 suites, 197 passed / 8 skipped / 205 total, Node 24.18), re-verified the export surface, TanStack cache keys, upload chunk bounds (5 MiB ≤ chunkSize ≤ 100 MiB, `URL_BATCH_SIZE` 50), the no-`exports`-map state, v0.1.0-alpha.5, and all five computational axiom checks; **found and fixed** a repo-wide `jest --coverage` tooling break (the root `pnpm.overrides` `glob: ">=10.5.0"` security pin fed glob 13 to `test-exclude@6` / `@jest/reporters` / `jest-config` / `jest-runtime`, which need glob v7's callable API — fixed with scoped `<pkg>>glob: ^7.2.3` overrides in the root `package.json`); with coverage now actually measurable, **`test:ci` is red**: real coverage is ~31% vs the 80% threshold — see §7) | **Meta version:** 0.6.0 (prev 0.4.0)
...
- **Tests** — `pnpm --filter @stoked-ui/media test` (Jest 29 + jsdom; run under Node 20, not Node 26). The **11** existing suites (verified 2026-06-06, re-verified green 2026-07-02: 197 passed, 8 skipped, 205 total) cover abstractions (`Auth`, `Router`, `Payment`, `Queue`, `KeyboardShortcuts`), `MediaCard` + MediaCard utils, `MediaViewer` + MediaViewer hooks (`useMediaViewerState`/`useMediaClassPlayback`), `WebUserDirectChat`, and an integration-backward-compatibility export smoke suite. **There is currently no `src/api/__tests__/` or `src/hooks/__tests__/`** — the API client, upload client, and React-Query hooks are guarded by the TypeScript build and downstream consumers, not by unit tests (these suites are *planned* in `SC_TEST.md`, not yet written). Coverage threshold is 80% (`jest.config.js`), enforced via `pnpm --filter @stoked-ui/media test:ci` — **but as of 2026-07-02 `test:ci` fails on the threshold itself**: measured coverage is ~31% statements/branches/lines and ~27% functions, because `collectCoverageFrom` counts the untested `src/api/`, `src/hooks/`, `src/server/`, `src/performance/`, and `src/zip/` trees. This was previously masked by a repo-wide coverage-tooling break (glob-override incompatibility, fixed 2026-07-02 via scoped `test-exclude>glob` / `@jest/reporters>glob` / `jest-config>glob` / `jest-runtime>glob` overrides in the root `package.json`); no one could have been running `test:ci` green. Closing the gap means writing the planned `src/api/__tests__/` and `src/hooks/__tests__/` suites (a governed task), **not** lowering the threshold (AX-MOD-SUIMEDIA-007 forbids that).
...
2. `pnpm --filter @stoked-ui/media test` — full Jest suite (green as of 2026-07-02). NB `test:ci` (coverage-enforced) is currently red at ~31% vs the 80% threshold; don't treat a `test:ci` failure as caused by your change unless coverage dropped below the pre-change baseline.
