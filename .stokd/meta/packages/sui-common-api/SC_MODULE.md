...
> **Generated:** 2026-05-05 (refreshed 2026-07-02 TIMED REFRESH; prev refreshes 2026-06-22 / 2026-06-06 / 2026-05-21) | **Meta version:** 0.6.0
...
> **Last code change:** 2026-07-02 refresh finds one **uncommitted working-tree change**: `product.model.ts` gained three install-distribution fields (`githubRepo`, `installPath`, `supportedOperatingSystems`) as part of the in-flight install-site work (`docs/pages/api/install/`, `docs/src/modules/products/install.ts`, `infra/install-site.ts`, `marketing/`). Last *committed* change touching this package's source remains the 2026-06-11 eslint/ts-fix sweep (`51a3baad04`); commit `13c8553b88` (2026-06-22) touched only `.axioms.md`. All other findings from the 2026-06-22 v0.6.0 re-verification still hold: same 12 models, both decorators, upload DTO, three barrels, root `index.ts`, and the `sui-media-api` `database.module.ts` registration of exactly `[VideoFeature, ImageFeature, FileFeature, UploadSessionFeature]`. Still **no committed tests and no `test` script** in `package.json`.
...
| Products Admin install-distribution fields + install-script endpoint (in-flight; not yet numbered in SC_VIEWS.md) | The new `Product.githubRepo` / `installPath` / `supportedOperatingSystems` props declare the fields the Products Admin form writes (via `docs/pages/api/products/index.ts` + normalizers in `docs/src/modules/products/install.ts`) and the projection `docs/pages/api/install/[script].ts` reads to stream install scripts for `install.stokd.cloud/<productId>.sh`. |
...
### Parallel raw-driver contract: the `products` collection (new, 2026-07-02)

`docs/pages/api/products/**` does **not** import this package — it reads/writes the `products` Mongo collection through the raw MongoDB driver (`getDb()` from `docs/src/modules/db/mongodb`). `src/models/product.model.ts` is therefore a **shape mirror**, not the runtime enforcer, of that collection: the docs admin API writes fields like `githubRepo`, `installPath`, and `supportedOperatingSystems` (normalized by `normalizeGithubRepo` / `normalizeInstallPath` / `normalizeSupportedOperatingSystems` in `docs/src/modules/products/install.ts`), and `docs/pages/api/install/[script].ts` projects `{ productId, name, live, githubRepo, installPath }` to serve install scripts. If `ProductFeature` is ever registered by a NestJS service against the same collection, any drift between this class and the raw-driver writes becomes a live bug. Keep the three surfaces (`product.model.ts`, `docs/src/modules/products/install.ts`, `docs/pages/api/products/**` + `docs/pages/api/install/**`) coordinated — see AX-MOD-SUICOMMONAPI-010.
...
8. **`Product` install-distribution shape mirror**: `Product.githubRepo` (owner/repo), `Product.installPath` (repo-relative), and `Product.supportedOperatingSystems` (`'macos' | 'linux' | 'windows'` values, per `SUPPORTED_OS_VALUES` in `docs/src/modules/products/install.ts`) must stay aligned with the docs raw-driver writes and the install-script endpoint projection (see the parallel raw-driver contract above).
...
| `src/models/product.model.ts` | Stripe product catalog. `productId` (unique index `product_id_unique`), `keyPrefix`, Stripe IDs, pricing/currency, `licenseDurationDays` (365), `gracePeriodDays` (14), `trialDurationDays` (30), `purchaseUrl`. **New (uncommitted, 2026-07-02):** install-distribution fields `githubRepo?` ("owner/repo"), `installPath?` (repo-relative script path), `supportedOperatingSystems?` (`string[]`, default `[]`; expected values `'macos'/'linux'/'windows'`). These mirror the raw-driver writes in `docs/pages/api/products/**` and the projection read by `docs/pages/api/install/[script].ts` (install.stokd.cloud). |
...
> **Committed stale compiled artifacts in `src/` (hazard, documented 2026-07-02):** the repo has **git-tracked** `*.js` + `*.js.map` files sitting next to their `.ts` sources (committed 2026-02-22 in `8fa2444861`): the root `src/index.js`, all of `src/decorators/` and `src/dtos/`, and the six *original* models (`base`, `file`, `image`, `video`, `media`, `uploadSession`) plus `src/models/index.js`. They are **stale**: `src/models/index.js` re-exports only those six models — `blogPost`, `invoice`, `license`, `product`, `client`, `user` are missing. `package.json#main` points at `src/index.ts`, so today's consumers (workspace TS compilation, the Babel `build/` pipeline) never load them; but any plain-Node/CJS resolver that prefers `.js` over `.ts` would silently see a package with six models missing. Treat these as candidates for deletion in a governed cleanup task; do not "update" them by hand. (Also note: `sst-env.d.ts` and `tsconfig.build.tsbuildinfo` in the package root are untracked local artifacts.)
...
| Edit `Product` install-distribution fields (`githubRepo`, `installPath`, `supportedOperatingSystems`) | These mirror the docs raw-driver `products` collection: validate against the normalizers in `docs/src/modules/products/install.ts` (`SUPPORTED_OS_VALUES`, `normalizeGithubRepo`, `normalizeInstallPath`), the admin write path `docs/pages/api/products/index.ts` / `[id].ts`, and the read projection in `docs/pages/api/install/[script].ts`. Renaming a field here without touching those surfaces silently desynchronizes the mirror (AX-MOD-SUICOMMONAPI-010). |
...
| Delete the stale committed `src/**/*.js(.map)` artifacts | Safe in principle (`main` is `src/index.ts`; the `build/` pipeline regenerates its own output), but do it as a governed cleanup task and re-run `pnpm --filter @stoked-ui/common-api build && pnpm --filter @stoked-ui/media-api typescript` to prove no resolver depended on them. |
...
5. (Install flow, once the in-flight work lands) Create/edit a product with `githubRepo` + `installPath` through the Products Admin and fetch `/api/install/<productId>.sh` — exercises the Product shape mirror end-to-end.
