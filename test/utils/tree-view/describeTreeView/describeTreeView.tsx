...
import { TreeItem, fileElementClasses } from '@mui/x-tree-view/TreeItem';
...
const innerDescribeTreeView = <TSignatures extends TreeViewAnyPluginSignature[]>(
  ...
  const getUtils = (result: MuiRenderResult): DescribeTreeViewRendererUtils => {
    ...
    const getFocusedItemId = () => {
      ...
      if (!activeElement || !activeElement.classList.contains(fileElementClasses.root)) {
        return null;
      }
    ...
    const getItemContent = (id: string) =>
      getItemRoot(id).querySelector<HTMLElement>(`.${fileElementClasses.content}`)!;

    const getItemCheckbox = (id: string) =>
      getItemRoot(id).querySelector<HTMLElement>(`.${fileElementClasses.checkbox}`)!;

    const getItemCheckboxInput = (id: string) =>
      getItemCheckbox(id).querySelector<HTMLInputElement>(`input`)!;

    const getItemLabel = (id: string) =>
      getItemRoot(id).querySelector<HTMLElement>(`.${fileElementClasses.label}`)!;

    const getItemIconContainer = (id: string) =>
      getItemRoot(id).querySelector<HTMLElement>(`.${fileElementClasses.iconContainer}`)!;
